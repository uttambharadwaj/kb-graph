...
```
src/
  mcp.js          ← MCP server (21 tools: search, write, capture, classify, safety)
...
| kb-checkpoint-hook.js | 26 | - | Thin installed PostToolUse checkpoint entry point. Keep module-load failures |
| kb-trigger-hook.js | 41 | - | bin/kb-trigger-hook.js — the installed PreToolUse (Bash) hook entry point. |
| kb.js | 328 | - | bin/kb.js — CLI entry point. |
...
| File | Lines | Exports | Purpose |
|------|-------|---------|---------|
| render-loop-figure.mjs | 156 | xmlEscape, wrapLine, computeLayout, renderLoopFigure, hasRenderDrift | - |
| smoke-tarball.sh | 185 | - | !/usr/bin/env bash |
| test-preflight.mjs | 71 | - | - |
...
| daemon-restart.js | 201 | DEFAULT_RESTART_GRACE_TIMEOUT_MS, DEFAULT_RESTART_MARKER_MAX_AGE_MS, DEFAULT_RESTART_MARKER_REFRESH_MS, restartMarkerPath, restartGenerationPath... | - |
| daemon.js | 469 | probeSocketDetailed, probeSocket, startDaemon | The resident KB service: one process, one unix socket, one MCP connection |
| db.js | 1617 | DEFAULT_BUSY_TIMEOUT_MS, MIGRATIONS, insertDocument, updateDocument, deleteDocument... | better-sqlite3's own default when no `timeout` option is passed — made |
...
| mcp.js | 27 | start | Allow direct execution |
| meters.js | 95 | METER_TABLES, EMPTY_REPLY_CHARS, PRUNE_EXCLUDED, PRUNABLE_TABLES, meterGrowth... | Retention for the five meter tables. Only extractions is currently |
...
| process-ancestry.js | 154 | AGENT, AGENTS, AGENT_FLAG, harnessAgent, findHarnessAncestor... | Identifies "the agent harness process" (Claude Code or Codex CLI) by |
...
| resident-census.js | 91 | summarizeResidentProcesses, inspectResidentProcesses, formatResidentProcessSummary | Live topology census for the resident-service rollout. Startup-event meters |
...
| retrieval.js | 242 | SURFACE, SURFACES, PUSH_SURFACES, READ_SURFACES, isKbNudge... | Read-path telemetry: the write path has always been logged (documents, |
...
| secret-prompt.js | 128 | askHidden | - |
| server.js | 229 | start | - |
| session-capture.js | 372 | SESSION_CAPTURE_QUEUE_DIR, SESSION_CAPTURE_RECEIPT_DIR, SESSION_CAPTURE_LOG, ensureSessionCaptureDirectories, writeJsonExclusive... | Durable, model-free handoff from lifecycle hooks to the resident daemon. |
...
| shim-hello.js | 91 | HELLO_KEY, HELLO_VERSION, DAEMON_PROBE_KEY, DAEMON_READY_KEY, DAEMON_PROBE_VERSION... | The one line `kb mcp-shim` writes before any JSON-RPC: which harness owns |
| shim-path-meter.js | 146 | SHIM_PATH_LOG, SHIM_PATH_WINDOW_MS, SHIM_RECOVERY_STAGES, recordShimPath, recordShimRecovery... | One row per mcp-shim startup decision. The fallback deliberately keeps KB |
...
| write-meter.js | 53 | WRITE_DECISION_SOURCE, logWriteDecision | One row per write decision: what the closest existing note was, how close, |
| write-note.js | 353 | RELATED_MIN, RELATED_K, WRITE_SKIP_REASON, renderRelatedSection, nearNeighborFields... | Shared note-writing path: dedup, frontmatter, related-links, index. |
...
| capture-follow-through.js | 576 | CAPTURE_COHORT, CAPTURE_STATE, CHECKPOINT_REPLAY_LABEL, CAPTURE_AGENT, CAPTURE_REASON... | Aggregate checkpoint outcomes without exposing the session-level evidence |
| checkpoint-hook.js | 457 | CHECKPOINT_REASON, CHECKPOINT_DECLINE_REASON, CHECKPOINT_MESSAGES, MAX_SESSION_CHECKPOINTS, CHECKPOINT_ENABLED_FLAG... | PostToolUse checkpoint hook: after a successful durable boundary, remind the |
...
| mcp-register.js | 274 | SUPPORTED_AGENTS, KB_MCP_SERVER_NAME, KB_ENTRYPOINT_PATH, mcpServerConfig, KB_MCP_SERVER_CONFIG... | Absent and invalid/unreadable are different answers. Treating either invalid |
| mcp-shim.js | 568 | STARTUP_READINESS_TIMEOUT_MS, PROBE_ATTEMPT_TIMEOUT_MS, CONNECT_TIMEOUT_MS, RECOVERY_HANDSHAKE_TIMEOUT_MS, RESTART_GRACE_TIMEOUT_MS... | Per-session stdio shim: connects this process's stdio to the resident |
...
| serve.js | 166 | startReplacementDaemon, isManagedRestartEnvironment, registerServeShutdown, runServeCli | - |
| session-capture-hook.js | 45 | sessionCaptureHook | Lifecycle hook entry: enqueue only. No extraction, summarization, indexing, |
| setup-hooks.js | 385 | HOOK_FILES, PUSH_AGENTS, mergeAgentHooks, installAgentHooks, unresolvableHookCommands... | src/cli/setup-hooks.js — install KB briefing/hint hooks into an agent's hook con |
| setup-jobs.js | 172 | xmlEscape, systemdEscape, renderPlist, renderSystemdUnits, installJobs | src/cli/setup-jobs.js — install harvest/reindex/synthesis as launchd or systemd  |
| setup.js | 789 | parseEnvFile, loadExistingEnv, askSecret, buildEnvContent, systemdServiceContent... | fileURLToPath handles Windows drive letters correctly (avoids C:\C:\ duplication |
| status.js | 42 | status | - |
| stop.js | 25 | stop | - |
| surface-report.js | 260 | modelCallDemand, surfaceReport, runSurfaceReportCli | What the newest meters have to say: which tools anyone actually calls, |
...
| tool-cli.js | 105 | FALLBACK_TOOL_NAMES, runToolCli | - |
...
| wakeup-hook.js | 240 | computeWakeupHook, commitWakeupHookPlan, wakeupHook | SessionStart hook: print a compact KB briefing to stdout so the harness |

## src/embeddings/

| File | Lines | Exports | Purpose |
|------|-------|---------|---------|
| embed.js | 145 | EMBEDDING_MODEL, EMBEDDING_DIMENSIONS, DEFAULT_EMBEDDING_LOAD_TIMEOUT_MS, resolveEmbeddingLoadTimeoutMs, resolveEmbeddingCacheDir... | Convert Float32Array to Buffer for SQLite BLOB storage (3x smaller than JSON) |
...
| v1.js | 287 | default | src/routes/v1.js |
...
| File | Lines | Exports | Purpose |
|------|-------|---------|---------|
| indexer.js | 380 | VAULT_INDEX_RACE_LOG, scanVault, indexVault, indexVaultFile, pruneMissingVaultFiles | `deferTriggerIndex`: skip the per-file rebuildTriggerIndex() call even when |
...
| auth-oauth-state.test.js | 141 | - | - |
| bus-removal.test.js | 91 | - | - |
| capture-embeddings.test.js | 45 | - | - |
| capture-follow-through.test.js | 551 | - | - |
...
| daemon.test.js | 490 | - | A listening server holds the event loop open, so a daemon a test failed to |
...
| harvest.test.js | 1372 | - | A claude that answers instantly, so the harvest runs end to end without the |
...
| hint-real-prompt-eval.test.js | 141 | - | - |
| hint-recall.test.js | 358 | - | The opposing force to hint-relevance.test.js. |
| hint-relevance.test.js | 462 | - | The prompt-hint surface used to fire on 100% of prompts — 94 of 94 logged |
...
| hooks-retrieval.test.js | 657 | - | Exercises wakeup-hook.js and prompt-hint.js as real subprocesses (they |
| hybrid-search.test.js | 71 | - | - |
| ingest.test.js | 110 | - | Ingested documents had no vault file, and the reindex job — which only walks |
| inverse-fold.test.js | 203 | - | Point the KB at a throwaway dir BEFORE importing anything that opens the DB. |
| loop-figure.test.js | 136 | - | - |
| mcp-annotations.test.js | 57 | - | Codex under approval_policy=never auto-approves only tools advertising |
| mcp-shim.test.js | 818 | - | Drives `kb mcp-shim` as a real child process against a real in-process |
| mcp-wire-identity.test.js | 194 | - | Captured by hand-rolled JSON-RPC against public/main (v1 SDK, pre-migration) |
| meter-retention.test.js | 263 | - | Isolated databases keep retention assertions independent from meter rows |
...
| server-bind.test.js | 414 | - | - |
| session-capture.test.js | 719 | - | - |
| session-map.test.js | 174 | - | Backdates a file's mtime by `days` so the sweeper's age check treats it as |
| setup-env-preserve.test.js | 31 | - | - |
| setup-hooks.test.js | 664 | - | tests/setup-hooks.test.js |
| setup-jobs.test.js | 154 | - | tests/setup-jobs.test.js |
| shim-hello.test.js | 169 | - | The compatibility direction step 2 cannot cover: a NEW shim dialing an OLD |
| shim-path-meter.test.js | 100 | - | - |
| source-hygiene.test.js | 45 | - | - |
| status-stop.test.js | 40 | - | - |
| supersession.test.js | 281 | - | - |
| surface-meters.test.js | 198 | - | - |
...
| vault-reindex-route.test.js | 72 | - | - |
| write-correction.test.js | 92 | - | - |
| write-fail-closed.test.js | 294 | - | - |
| write-lock-process.test.js | 74 | - | - |
| write-note.test.js | 85 | - | - |
