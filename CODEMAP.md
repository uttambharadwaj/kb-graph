...
| extract.js | 467 | EXTRACT_PROMPT, MAX_EXTRACT_CHARS, buildExtractPrompt, chunkForExtract, extractFacts... | Auto-capture: turn a raw work conversation / session transcript into durable |
...
| harvest.js | 353 | MAX_SESSIONS_PER_RUN, factsRequested, LESSONS_PROMPT, isPrintModeTranscript, harvestsPrintModeSessions... | Nightly auto-debrief: sweep agent session transcripts (Claude Code, and |
...
| extract.test.js | 567 | - | Point the KB at a throwaway dir BEFORE importing anything that opens the DB. |
...
| harvest.test.js | 294 | - | A claude that answers instantly, so the harvest runs end to end without the |
