...
**Facts:** call `kb_extract` once with the session's relevant text (decisions, state changes, ownership, incidents) plus `source` and `observation_date` — it extracts and consolidates triples, retiring a contradicted fact only where the predicate is single-valued (`status`, `merged_via`, and the rest of `src/predicates.json`) — everything else, including `owns` and `deployed_to`, keeps both objects and needs a manual `kb_fact_invalidate` when the old one really is dead. Use `dry_run: true` to preview. Read `skipped` for assertions it chose not to record, then fill gaps with manual `kb_fact_add`; retire superseded facts with `kb_fact_invalidate`.
