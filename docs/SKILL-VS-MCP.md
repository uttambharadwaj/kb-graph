...
**What it is:** A set of 26 tools your AI agent can call on-demand to search, read, write, and manage your knowledge base. 19 are also exposed over HTTP; 7 administrative or local-only tools are stdio-only.
...
Without the skill, your agent has 26 tools but no strategy for when to use them. It might:
...
| | MCP Server | Skill |
|---|---|---|
| **What** | 26 tools (search, read, write, capture...) | Instructions for using those tools efficiently |
...
cp -r skills/kb-workflow/ ~/.claude/skills/kb-workflow/
# Or symlink it
ln -s $(pwd)/skills/kb-workflow ~/.claude/skills/kb-workflow
```
