...
//
// The matcher itself (stripHeredocs, commandSegments, patternMatchesSegment,
// matchCommand, loadTriggerIndex, and the two path constants) lives in
// trigger-match.js, which has no db.js import — the PreToolUse hook
// (src/cli/trigger-hook.js) runs on every Bash call and must not carry the
...
import {
  ...
  patternMatchesSegment, matchCommand, loadTriggerIndex,
} from './trigger-match.js';

export { CORPUS_PATH, TRIGGER_INDEX_PATH, stripHeredocs, commandSegments, matchCommand, loadTriggerIndex };
