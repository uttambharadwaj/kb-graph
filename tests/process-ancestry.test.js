...
// named "claude-hook"), and against a live Codex CLI tree (npm-installed
// vendor binary plus its own code-mode host). harnessAgent must name exactly
// the CLI binary layers and nothing else in either tree.
describe('harnessAgent', () => {
  const cases = [
    ['/Users/uttambharadwaj/.local/bin/claude', AGENT.CLAUDE],
    ['/Users/uttambharadwaj/.local/share/claude/ClaudeCode.app/Contents/MacOS/claude', AGENT.CLAUDE],
    ['claude', AGENT.CLAUDE], // bg-pty-host/bg-spare children: argv0 rewritten to a bare name
    ['claude bg-pty-host', null], // argv0 + args landing in comm on some ps builds — basename check is exact
    ['/Users/uttambharadwaj/.local/share/claude/versions/2.1.226', null], // versioned binary, argv0 is the version
    ['/Applications/Claude.app/Contents/MacOS/Claude', null], // desktop app: capital C
    ['/Applications/Claude.app/Contents/Frameworks/Claude Helper (Renderer).app/Contents/MacOS/Claude Helper (Renderer)', null],
    ['cmux', null], // orchestrator that shells out to a command named "claude-hook"
    ['/opt/homebrew/Cellar/node@22/22.23.1/bin/node', null],
    // Live Codex CLI, captured the same way (2026-08-23).
    ['/opt/homebrew/lib/node_modules/@openai/codex/node_modules/@openai/codex-darwin-arm64/vendor/aarch64-apple-darwin/bin/codex', AGENT.CODEX],
    ['codex', AGENT.CODEX],
    ['/opt/homebrew/lib/node_modules/@openai/codex/node_modules/@openai/codex-darwin-arm64/vendor/aarch64-apple-darwin/bin/codex-code-mode-host', null],
    ['/Applications/ChatGPT.app/Contents/Frameworks/Codex Framework.framework/Versions/151.0.7922.137/Helpers/Codex (Renderer).app/Contents/MacOS/Codex (Renderer)', null],
    [null, null],
    ['', null],
  ];
  for (const [comm, expected] of cases) {
    it(`${JSON.stringify(comm)} -> ${expected}`, () => {
      assert.strictEqual(harnessAgent(comm), expected);
    });
  }
});
...
describe('findHarnessAncestor', () => {
  const LSTART = 'Sun Aug  9 20:00:00 2026';
  const row = (pid, ppid, comm, lstart = LSTART) => ({ pid, ppid, comm, lstart });

  it('finds a harness ancestor one hop up (the common, non-nested case) and returns its row', () => {
    const table = [
    ...
    assert.deepStrictEqual(findHarnessAncestor(200, table), row(100, 1, '/Users/uttambharadwaj/.local/bin/claude', 'Sun Aug  9 19:00:00 2026'));
  });
  ...
  it('skips a non-matching intermediate layer to find the CLI wrapper further up (daemon nesting)', () => {
    ...
    assert.strictEqual(findHarnessAncestor(300, table)?.pid, 100);
  });
  ...
  it('returns null when no ancestor matches (e.g. a desktop-app-spawned MCP subprocess)', () => {
    ...
    assert.strictEqual(findHarnessAncestor(400, table), null);
  });

  it('returns null for a pid not present in the table', () => {
    assert.strictEqual(findHarnessAncestor(999, []), null);
  });

  it('does not loop forever on a cyclic ppid chain', () => {
    const table = [row(1, 2, 'node'), row(2, 1, 'node')];
    assert.strictEqual(findHarnessAncestor(1, table), null);
  });
});

describe('resolveHarnessAncestry', () => {
  it('returns the matched pid and its start time on success, from one ps call', () => {
    const result = resolveHarnessAncestry({
      pid: 50,
      listProcesses: () => [
    ...
    assert.deepStrictEqual(result, { harnessPid: 10, pidStart: 'Sun Aug  9 20:00:00 2026', agent: AGENT.CLAUDE });
  });

  it('names codex when the harness ancestor is the Codex CLI', () => {
    const result = resolveHarnessAncestry({
      pid: 50,
      listProcesses: () => [
        { pid: 50, ppid: 10, comm: 'node', lstart: 'irrelevant-for-node' },
        { pid: 10, ppid: 1, comm: '/opt/homebrew/lib/node_modules/@openai/codex/node_modules/@openai/codex-darwin-arm64/vendor/aarch64-apple-darwin/bin/codex', lstart: 'Sat Aug 22 17:30:03 2026' },
      ],
    });
    assert.deepStrictEqual(result, { harnessPid: 10, pidStart: 'Sat Aug 22 17:30:03 2026', agent: AGENT.CODEX });
  });

  it('takes the NEAREST harness ancestor when both are in the chain (a codex session running claude, or the reverse)', () => {
    const result = resolveHarnessAncestry({
      pid: 50,
      listProcesses: () => [
        { pid: 50, ppid: 20, comm: 'node', lstart: 'x' },
        { pid: 20, ppid: 10, comm: '/usr/local/bin/codex', lstart: 'Sat Aug 22 18:00:00 2026' },
        { pid: 10, ppid: 1, comm: '/usr/local/bin/claude', lstart: 'Sat Aug 22 17:00:00 2026' },
      ],
    });
    assert.deepStrictEqual(result, { harnessPid: 20, pidStart: 'Sat Aug 22 18:00:00 2026', agent: AGENT.CODEX });
  });
  ...
  it('returns nulls when no ancestor matches', () => {
    ...
    assert.deepStrictEqual(result, { harnessPid: null, pidStart: null, agent: null });
  });
