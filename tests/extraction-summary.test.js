...
describe('extraction status summary', () => {
  it('reports the last 24h denominator and rejection categories', () => {
    insert.run('recent-a', 3, 4, 1, 1, 1, 1, 2, 3, 4, 1200, 8, 0, '2026-08-26 11:00:00');
    insert.run('recent-b', 2, 1, 0, 0, 1, 0, 4, 5, 2, 900, 12, 1, '2026-08-26 10:00:00');
    insert.run('old', 99, 99, 99, 99, 99, 99, 99, 99, 99, 9999, 9999, 1, '2026-08-24 10:00:00');
    ...
    assert.deepStrictEqual(summary, {
      ...
      attempts: 6,
      slowest_model_ms: 1200,
      slowest_consolidation_ms: 12,
      chunk_failures: 1,
      failed: 1,
    });
    assert.equal(
      formatExtractionSummary(summary),
      'extractions (last 24h): 2 calls, 5 emitted, 5 skipped (entity 1, claim 2, date override 1, other 1), reconciled 6 duplicate skips and 8 accepted-and-skipped conflicts, 6 model attempts, slowest phases model 1200ms / consolidation 12ms, 1 chunk failure, 1 failed call',
    );
  });
