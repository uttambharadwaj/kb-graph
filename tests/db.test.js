...
describe('vault_files schema', () => {
  ...
  it('should create indexes for common document list and lookup paths', () => {
    const indexes = db.prepare(
      "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='documents'"
    ).all().map(row => row.name);

    assert.ok(indexes.includes('idx_documents_created_at'));
    assert.ok(indexes.includes('idx_documents_type_created_at'));
    assert.ok(indexes.includes('idx_documents_source'));
  });
});
