...
describe('kb_fact_add retires a contradicted single-valued fact', () => {
  it('the verbatim TKT-3469 repro: shipped -> in_review retires shipped', async () => {
    await tool('kb_fact_add').handler({
      subject: 'tkt-3453', predicate: 'status', object: 'shipped', valid_from: '2026-08-11',
    });
    assert.deepStrictEqual(current('tkt-3453', 'status'), ['shipped']);

    const res = stored(await tool('kb_fact_add').handler({
      subject: 'tkt-3453', predicate: 'status', object: 'in_review', valid_from: '2026-08-11',
    }));

    // Both current was the bug: only in_review may be current now.
    assert.deepStrictEqual(current('tkt-3453', 'status'), ['in_review']);
    ...
    const all = queryFact('tkt-3453', { direction: 'outgoing', exact: true });
  ...
  it('same-day retirement is accepted, not refused as ending before it began', async () => {
    ...
    await tool('kb_fact_add').handler({
      subject: 'tkt-5001', predicate: 'status', object: 'open', valid_from: '2026-08-11',
    });
    const res = stored(await tool('kb_fact_add').handler({
      subject: 'tkt-5001', predicate: 'status', object: 'closed', valid_from: '2026-08-11',
    }));

    assert.strictEqual(res.retired.length, 1);
    assert.strictEqual(res.retired[0].valid_to, '2026-08-11');
    assert.deepStrictEqual(current('tkt-5001', 'status'), ['closed']);
  });
  ...
  it('a many-valued predicate on a ticket subject also keeps both (accumulate, not retire)', async () => {
    await tool('kb_fact_add').handler({
      subject: 'tkt-5002', predicate: 'chose', object: 'embeddings at write time',
    });
    const res = stored(await tool('kb_fact_add').handler({
      subject: 'tkt-5002', predicate: 'chose', object: 'restart on source change',
    }));

    assert.strictEqual(res.retired, undefined);
    assert.strictEqual(current('tkt-5002', 'chose').length, 2);
  });
  ...
  it('retirement targets the row\'s own stored predicate spelling, not the canonical one', () => {
    ...
    const db = getDb();
    const subId = entityKey('tkt-5003');
    const objId = entityKey('legacy-open');
    db.prepare('INSERT OR IGNORE INTO entities (id, name) VALUES (?, ?)').run(subId, 'tkt-5003');
    ...
    return (async () => {
      const res = stored(await tool('kb_fact_add').handler({
        subject: 'tkt-5003', predicate: 'status', object: 'closed', valid_from: '2026-08-11',
      }));
      ...
      assert.deepStrictEqual(current('tkt-5003', 'status'), ['closed']);
  ...
  it('a fact that does not contradict anything writes with no retired field (no behavior change)', async () => {
    const res = stored(await tool('kb_fact_add').handler({
      subject: 'tkt-5004', predicate: 'status', object: 'open', valid_from: '2026-08-11',
    }));
    assert.strictEqual(res.retired, undefined);
  ...
  it('addFact itself gains no retirement of its own — the rule lives one layer up, at the tool', () => {
    ...
    addFact('tkt-5005', 'status', 'open', { validFrom: '2026-08-11' });
    addFact('tkt-5005', 'status', 'closed', { validFrom: '2026-08-11' });
    assert.deepStrictEqual(current('tkt-5005', 'status').sort(), ['closed', 'open']);
  });
  ...
  it('a backdated valid_from does not retire the newer row, and the fact is not written at all', async () => {
    ...
    // a different angle.
    await tool('kb_fact_add').handler({
      subject: 'tkt-6001', predicate: 'status', object: 'shipped', valid_from: '2026-08-11',
    });

    const res = stored(await tool('kb_fact_add').handler({
      subject: 'tkt-6001', predicate: 'status', object: 'in_review', valid_from: '2026-08-01',
    }));
    ...
    assert.deepStrictEqual(current('tkt-6001', 'status'), ['shipped']);
    // And no in_review row of any kind was written, current or otherwise.
    const all = queryFact('tkt-6001', { direction: 'outgoing', exact: true });
  ...
  it('a forward-dated (or same-day) value still retires and writes normally', async () => {
    await tool('kb_fact_add').handler({
      subject: 'tkt-6002', predicate: 'status', object: 'shipped', valid_from: '2026-08-01',
    });

    const res = stored(await tool('kb_fact_add').handler({
      subject: 'tkt-6002', predicate: 'status', object: 'in_review', valid_from: '2026-08-11',
    }));
